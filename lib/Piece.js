...
Piece.prototype.init = function(canvas_,uiCanvas_,red,green,blue,alpha,actions,randoms,life,smearDuration,size){
  this.canvas = canvas_, this.uiCanvas = uiCanvas_;
  this.red = red, this.green = green, this.blue = blue, this.alpha = alpha;
  this.actions = actions, this.randoms = randoms, this.life = life, this.smearDuration = smearDuration, this.size = size;
...
Piece.prototype.uiAddBrush = function(action,what,x,y,vel,random){
  ...
  manager.addBrush(new Brush(x-size/2,y,x+size/2,y,vel,Number(this.life.value),Number(this.smearDuration.value)));
  var newBrush = manager.brushes[manager.brushes.length-1];
  newBrush.setMaxSpeedAndForce(.5,.01);
  newBrush.rgbaValues = [Number(self.red.value),Number(self.green.value),Number(self.blue.value),Number(self.alpha.value)];
}

Piece.prototype.clickInterface = function(){
  var self = this;
  this.uiCanvas.onclick = function(event){
    var rect = self.canvas.getBoundingClientRect(), root = document.documentElement;
    var x = event.clientX - rect.left - root.scrollLeft - 10;
    var y = event.clientY - rect.top - root.scrollTop - 10;
    if(!self.addingBrush){
      self.x = x;
      self.y = y;
  ...
  document.onmousemove = function(event){
    var rect = self.canvas.getBoundingClientRect(), root = document.documentElement;
    self.mx = event.clientX - rect.left - root.scrollLeft - 10;
    self.my = event.clientY - rect.top - root.scrollTop - 10;
  }
  if(self.addingBrush){
      var ctx = self.uictx;
