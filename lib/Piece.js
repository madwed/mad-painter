...
Piece.prototype.init = function(canvas_,red,green,blue,alpha,actions,randoms,life,size){
  this.canvas = canvas_, this.red = red, this.green = green, this.blue = blue, this.alpha = alpha;
  this.actions = actions, this.randoms = randoms, this.life = life, this.size = size;
...
Piece.prototype.uiAddBrush = function(action,what,x,y,vel,life,random){
  var type = undefined;
  var name = "";
  if(action == "smear"){
    switch (what){
  ...
  manager.addBrush(new Brush(x-10-size/2,y-10,x-10+size/2,y-10,vel,life));
...
Piece.prototype.clickInterface = function(){
  ...
  this.canvas.onclick = function(event){
    ...
    if(!self.addingBrush){
      ...
      self.uiAddBrush(method[0],method[1],self.x,self.y,vel,self.life.value,random);  
    }
  }
}
