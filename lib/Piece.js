...
Piece.prototype.initUI = function(actions,randoms){
  ...
  var uiCanvas = add('CANVAS',{id:'uiCanvas',class:'canvas',width:this.width,height:this.height});
  this.uiCanvas = canvasBox.appendChild(uiCanvas);
  this.uictx = this.uiCanvas.getContext('2d');
  //Adding UI buttons
  var container = canvasBox.parentNode;
  var buttons = add('DIV',{id:'buttons'});
  //Color Selector
  var swatch = add('DIV',{id:'swatch',class:'ui-widget-content ui-corner-all'});
  buttons.appendChild(swatch);
  var colors = add('DIV',{id:'colors'});
  var red = add('DIV',{id:'red'}), green = add('DIV',{id:'green'}), blue = add('DIV',{id:'blue'}), alpha = add('DIV',{id:'alpha'});
  ...
  //Randomness Selector
  var rand = add('DIV',{id:'rand'});
  var randUnder = add('P',{class:'title'}), randOver = add('P',{class:'title'}), randOff = add('P',{class:'title'});
  randUnder.textContent = "randomize under color values";
  var randUnderInput = add('INPUT',{name:'random',value:'down',type:'radio'});
  randUnder.appendChild(randUnderInput);
  randOver.textContent = "randomize over color values";
  var randOverInput = add('INPUT',{name:'random',value:'up',type:'radio'});
  randOver.appendChild(randOverInput);
  randOff.textContent = "don't randomize";
  var randOffInput = add('INPUT',{name:'random',value:'no',type:'radio',checked:'checked'});
  ...
  //Brush Properties
  var brush = add('DIV',{id:'brush'});
  var brushProperties = add('DIV',{id:'brushProperties'});
  //Brush Props
  var brushProps = add('DIV',{id:'brushProps'});
  var strokeLength = add('P',{class:'title'}); strokeLength.textContent = 'stroke length';
  var life = add('DIV',{id:'life'});
  var smearLength = add('P',{class:'title'}); smearLength.textContent = 'smear length';
  var smear = add('DIV',{id:'smear'});
  var brushWidth = add('P',{class:'title'}); brushWidth.textContent = 'brush width';
  var size = add('DIV',{id:'size'});
  ...
  //Brush Spins
  var brushSpins = add('DIV',{id:'brushSpins'});
  var p1 = document.createElement('P'); 
  var lifeSpin = add('INPUT',{id:'lifeSpin'});
  var p2 = document.createElement('P');
  var smearSpin = add('INPUT',{id:'smearSpin'});
  var p3 = document.createElement('P');
  var sizeSpin = add('INPUT',{id:'sizeSpin'});
  ...
  //Brush Type
  var brushTypeLabel = add('LABEL',{for:'brushType'}); brushTypeLabel.textContent = 'Select a brush';
  brush.appendChild(brushTypeLabel);
  var brushTypeSelect = add('SELECT',{name:'brushType',id:'brushType'});
  var wet = add('OPTION',{selected:'selected'}); wet.textContent = 'Wet';
  var dry = document.createElement('OPTION'); dry.textContent = 'Dry';
  ...
  //Managers
  var managersWrapper = add('DIV',{id:'managers-wrapper'});
  var managers = add('DIV',{id:'managers'});
  var h3Blend = document.createElement('H3'); h3Blend.textContent = 'blend';
  managers.appendChild(h3Blend);
  var blend = document.createElement('DIV');
  var blendDefault = document.createElement('P'); blendDefault.textContent = 'default';
  var blendDefaultInput = add('INPUT',{name:'action',value:'blend default',type:'radio'});
  blendDefault.appendChild(blendDefaultInput);
  blend.appendChild(blendDefault);
  var blendBW = document.createElement('P'); blendBW.textContent = 'b/w';
  var blendBWInput = add('INPUT',{name:'action',value:'blend b/w',type:'radio'});
  blendBW.appendChild(blendBWInput);
  blend.appendChild(blendBW);
  managers.appendChild(blend);

  var h3Smear = document.createElement('H3'); h3Smear.textContent = 'smear';
  managers.appendChild(h3Smear);

  var smear = document.createElement('DIV');
  var smearAll = document.createElement('P'); smearAll.textContent = 'all';
  var smearAllInput = add('INPUT',{name:'action',value:'smear default',type:'radio'});
  smearAll.appendChild(smearAllInput);
  smear.appendChild(blendDefault);

  var smearOpaque = document.createElement('P'); smearOpaque.textContent = 'opaque';
  var smearOpaqueInput = add('INPUT',{name:'action',value:'smear opaque',type:'radio', checked:'checked'});
  smearOpaque.appendChild(smearOpaqueInput);
  smear.appendChild(smearOpaque);

  var smearAntiTransparent = document.createElement('P'); smearAntiTransparent.textContent = 'antiTransparent';
  var smearAntiTransparentInput = add('INPUT',{name:'action',value:'smear antiTransparent',type:'radio'});
  smearAntiTransparent.appendChild(smearAntiTransparentInput);
  smear.appendChild(smearAntiTransparent);

  var smearAwayDark = document.createElement('P'); smearAwayDark.textContent = 'awayDark';
  var smearAwayDarkInput = add('INPUT',{name:'action',value:'smear awayDark',type:'radio'});
  smearAwayDark.appendChild(smearAwayDarkInput);
  smear.appendChild(smearAwayDark);
  var smearAwayLight = document.createElement('P'); smearAll.textContent = 'awayLight';
  var smearAwayLightInput = add('INPUT',{name:'action',value:'smear awayLight',type:'radio'});
...
Piece.prototype.clickInterface = function(){
  ...
  $("#uiCanvas").on("click",function(event){
