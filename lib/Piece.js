...
Piece.prototype.uiAddBrush = function(action,what,x,y,vel,life,rgba,random){
  var type = undefined;
  var name = "";
  if(action == "smear"){
    switch (what){
...
Piece.prototype.clickInterface = function(){
  ...
  this.canvas.onclick = function(event){
    ...
    if(!self.addingBrush){
      ...
      self.uiAddBrush(method[0],method[1],self.x,self.y,vel,self.life.value,
                        [Number(self.red.value),Number(self.green.value),Number(self.blue.value),Number(self.alpha.value)],random);  
    }
  }
}
