...
PixelBrush = function(x1_,y1_,x2_,y2_,brushSys_,type_){
  ...
  //this.getPixelsNoGap();
  //for (var i=0; i < this.pixelLine.length; i++){
    //this.getPixelRGBFromCenter();
  //}
...
PixelBrush.prototype.clearPixelLine = function(){
  this.pixelLine = [];
}
...
PixelBrush.prototype.getPixelRGB = function(pixInLine){
  if(this.pixelLine[pixInLine] === -1){
    ...
    this.pixelRGB[pixInLine*4  ] = this.brushSys.iData.data[this.pixelLine[pixInLine]*4  ];
    this.pixelRGB[pixInLine*4+1] = this.brushSys.iData.data[this.pixelLine[pixInLine]*4+1];
    this.pixelRGB[pixInLine*4+2] = this.brushSys.iData.data[this.pixelLine[pixInLine]*4+2];
    this.pixelRGB[pixInLine*4+3] = this.brushSys.iData.data[this.pixelLine[pixInLine]*4+3];
  }
}
...
PixelBrush.prototype.greenPix = function(){
  for (var grPi=0;grPi<this.pixelLine.length;grPi++){
    if(this.pixelLine[grPi]>=0){
      this.brushSys.iData.data[this.pixelLine[grPi]*4    ] = 0;
      this.brushSys.iData.data[this.pixelLine[grPi]*4 + 1] = 100;
      this.brushSys.iData.data[this.pixelLine[grPi]*4 + 2] = 0;
...
PixelBrush.prototype.drawLineRGB = function(){
  for (var pix=0;pix<this.pixelLine.length;pix++){
    if(this.smearingArray[pix]){
      this.brushSys.ctx.lineWidth = 1;
      this.grd = this.brushSys.ctx.createLinearGradient(this.verts[0].x,
                              this.verts[0].y,
                              this.verts[1].x,
                              this.verts[1].y);
      var gx = 1 / this.pixelLine.length;

      for (var i=0; i < this.pixelLine.length; i++){
        this.grd.addColorStop(i*gx,rgbString(this.pixelRGB[i*4],
            this.pixelRGB[i*4+1],
            this.pixelRGB[i*4+2],
            this.pixelRGB[i*4+3]));
      }
      ...
      this.brushSys.ctx.beginPath();
      this.brushSys.ctx.moveTo(this.verts[0].x,this.verts[0].y);
      this.brushSys.ctx.lineTo(this.verts[1].x,this.verts[1].y);
      this.brushSys.ctx.strokeStyle = this.grd;
      this.brushSys.ctx.fillStyle = this.grd;
      this.brushSys.ctx.stroke();
...
PixelBrush.prototype.isItSmearing = function(num){
  if(this.smearing=true){
    this.smearing= true;
    this.smearsRemaining = num;
...
PixelBrush.prototype.smear = function(){
  if(this.smearing){
    this.smearsRemaining[pix]--;
    if(this.smearsRemaining == 0){
      this.smearing= false;
    }
    return true
  }
...
PixelBrush.prototype.smearAgainstDark = function(){
  for (var pix=0;pix<this.pixelLine.length;pix++){
    if(this.smearingArray[pix]){
