...
BrushManager.prototype.getMarksNoGaps = function(){
  ...
  this.brushes.forEach(function(brush, index){
	...
	for(var bristle = 1; bristle < bristles; bristle++){
		...
		if(oldAxis !== newAxis){pixels.push(utils.getPixel(flatAxis, oldAxis, this.width, this.height)); }
		pixels.push(utils.getPixel(flatAxis, newAxis, this.width, this.height));
	}
	marks.pixels[index] = pixels; marks.rGBAs[index] = brush.rgbaValues.splice();
	}, this);
	return marks;
};
