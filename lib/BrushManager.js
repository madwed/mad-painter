...
BrushManager.prototype.run = function(){
	this.checkDeath();
  	...
  	var getMark = self.brushType;
   	markMethod(this.chargeBrushes(getMark(self)),this);
}
  ...
  self.brushes.forEach(function(brush,index){
	  ...
	  if(dx > dy){
	  	...
	  	for (var bristle = 1; bristle < bristles; bristle++){
    		...
    		if(oldY != newY){
        		pixels.push(utils.getPixel(x,oldY,self.width,self.height));
      		}
      		pixels.push(utils.getPixel(x,newY,self.width,self.height));
   		}
	  }else{
	  	pixels.push(utils.getPixel(bristlePositions[0][0],bristlePositions[0][1]));
	  	for (var bristle = 1; bristle < bristles; bristle++){
    		...
    		if(oldX != newX){
        		pixels.push(utils.getPixel(oldX,y,self.width,self.height));
      		}
      		pixels.push(utils.getPixel(newX,y,self.width,self.height));
   		}
	  }
	  marks.pixels[index] = pixels;
  });
